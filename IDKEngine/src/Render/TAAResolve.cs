    ...
    unsafe class TAAResolve : IDisposable
    {
        ...
        public void SetSize(int width, int height)
        {
...

            {
                ...
                FSR2.ContextDescription.DelegateMessage delegateMessage = Fsr2Message;

                if (isFsr2Initialized)
                {
                    FSR2.ContextDestroy(out fsr2Context);
                }

                FSR2.ContextDescription contextDesc = new FSR2.ContextDescription
                {
                    Flags = FSR2.InitializationFlagBits.EnableDebugChecking | FSR2.InitializationFlagBits.EnableAutoExposure | FSR2.InitializationFlagBits.AllowNullDeviceAndCommandList,
                    MaxRenderSize = new FSR2Types.Dimensions2D() { Width = (uint)width, Height = (uint)height },
                    DisplaySize = new FSR2Types.Dimensions2D() { Width = (uint)width, Height = (uint)height },
                ...
                FSR2.GL.GetInterface(out contextDesc.Callbacks, ref fsr2ScratchMemory[0], fsr2ScratchMemory.Length, GLFW.GetProcAddress);
